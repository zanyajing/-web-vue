			...
			<div class="login-logo">
				...
				<div class="login-cont">
					<div class="form" style="position:relative;">
						...
						<div class="item">
							<input type="text" class="text-field" name="" id="" maxlength="11" v-model="mobile" value="" placeholder="请输入手机号" />
						</div>
						<!--<router-link to="/passwordRet" @click="passwordCode" class="btn-field">找回密码> </router-link>-->
						<!--<div id="embed-captcha"></div>-->
						<a href="javascript:;" @click="passwordCode" class="btn-field">下一步</a>
						<div class="login-hav">
  ...
  export default {
		...
		methods: {
			...
			sendVeirfyCode() { //发送验证码
					...
					sendVeirfyCode({
						mobile: this.mobile,
						type: 'SEND_BY_REGIST_ACCOUNT'
					}).then((res) => {
					  this.canClick = true;
						if(ERR_OK === res.code)
						this.$router.push({name:'registerPwd',query:{mobile:this.mobile}});
					})
			},
			_getCaptcha() {
				var _this = this
